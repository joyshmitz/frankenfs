...
| `bd-rchk0.2.4` | Writeback-cache operator documentation | Complete: README limitations/status, `ffs mount --help` comments, and `docs/design-writeback-cache-mvcc.md` now agree that kernel `writeback_cache` is default-off, explicit read-write opt-in only, and gated by accepted audit, ordering, crash/replay, runtime-guard, host/lane, and kill-switch evidence. Operator docs cite validator commands, unit-test groups, E2E scenario IDs, crash/replay artifact fields, and the remaining readiness deferrals (`bd-rchk3`, `bd-rchk4`, `bd-rchk5`, `bd-rchk0.5.9`/`bd-t21em`). |
