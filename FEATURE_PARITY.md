...
| btrfs superblock decode | `fs/btrfs/disk-io.c` | ✅ | Implemented in `ffs-ondisk`, including sector/node/stripe size-field validation, root/chunk/log tree-level validation against the kernel maximum level, and fail-closed checksum-type validation for the currently supported CRC32C superblock format. |
...
| btrfs item payload decode (ROOT/INODE/DIR/EXTENT_DATA) | `fs/btrfs/ctree.c` | ✅ | Implemented in `ffs-btrfs` (`parse_root_item`, `parse_root_ref`, `parse_inode_item`, `parse_dir_items`, `parse_extent_data`) with direct cargo-fuzz coverage in `fuzz_btrfs_tree_items` for deterministic decode and field-level payload invariants. `parse_extent_data()` rejects unsupported extent compression values and nonzero encryption/other-encoding fields instead of treating encoded payloads as raw readable data. |
...
| btrfs device tree discovery | `fs/btrfs/volumes.c`, `fs/btrfs/ioctl.c` | ✅ | `walk_device_tree()` in ffs-btrfs walks device tree to enumerate all physical devices. Harness integration coverage in `crates/ffs-harness/tests/conformance.rs` now verifies multilevel device-tree traversal returns both discovered `DEV_ITEM` payloads and that each parses into the expected device metadata, with zero-capacity devices and impossible `bytes_used > total_bytes` accounting rejected at the on-disk parser boundary. `BTRFS_IOC_DEV_INFO` is wired through the FUSE ioctl surface, and `crates/ffs-harness/tests/fuse_e2e.rs` now proves mounted-path wildcard, exact `devid+uuid`, and missing-device `ENODEV` contracts for the image-backed single-device case. |
