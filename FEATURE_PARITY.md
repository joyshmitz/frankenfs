...
| FUSE mount background scrub lifecycle | FrankenFS spec §4 / §9 | ✅ | `ffs mount` now starts a detection-only `ffs-repair::ScrubDaemon` automatically for default read-only mounts, owns cancellation through a mount lifecycle guard, joins the worker on shutdown, and emits structured start/stop/error logs. CLI controls cover `--background-scrub`, `--no-background-scrub`, `--background-scrub-interval-secs`, `--background-scrub-ledger`, and `--background-repair`; the new repair flag implies scrub, requires a durable ledger, checks writable backing-image access, and runs the real recovery/writeback plus repair-symbol refresh path for client read-only mounts. Read-write mounts keep background scrub disabled by default unless explicitly opted into detection-only monitoring, and `--background-repair` is rejected with `--rw` until repair writeback is serialized with mounted write traffic. Coverage: `crates/ffs-cli/src/main.rs` mount config/start-stop tests and `crates/ffs-repair/src/pipeline.rs` repair-enabled plus detection-only daemon tests. |
