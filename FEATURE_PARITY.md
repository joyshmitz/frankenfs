...
| FUSE mount background scrub lifecycle | FrankenFS spec §4 / §9 | ✅ | `ffs mount` now starts a detection-only `ffs-repair::ScrubDaemon` automatically for default read-only mounts, owns cancellation through a mount lifecycle guard, joins the worker on shutdown, and emits structured start/stop/error logs. CLI controls cover `--background-scrub`, `--no-background-scrub`, `--background-scrub-interval-secs`, `--background-scrub-ledger`, and `--background-repair`; the repair flag implies scrub, requires a durable ledger, checks writable backing-image access, and runs the recovery/writeback plus repair-symbol refresh path for client read-only mounts through an explicit direct-device authority. Read-write mounts keep background scrub disabled by default unless explicitly opted into detection-only monitoring, and `--background-repair` is rejected with `--rw` unless callers use the mounted MVCC repair writeback authority and serialization contract. Coverage: `crates/ffs-cli/src/main.rs` mount config/start-stop tests, `crates/ffs-core/src/lib.rs` mounted repair-writeback request-scope tests, and `crates/ffs-repair/src/pipeline.rs` repair-enabled, custom-authority, rejection, and detection-only daemon tests. |
