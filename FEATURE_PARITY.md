...
| btrfs superblock decode | `fs/btrfs/disk-io.c` | ✅ | Implemented in `ffs-ondisk`, including sector/node/stripe size-field validation, zero-capacity and impossible `bytes_used > total_bytes` accounting rejection, root/chunk/log tree-level validation against the kernel maximum level, and fail-closed checksum-type validation for the currently supported CRC32C superblock format. |
