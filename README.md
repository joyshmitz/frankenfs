...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-7%2C419%20entries-brightgreen" alt="7,419 tests">
...
# Read-write mount with mounted automatic repair + evidence ledger
sudo cargo run -p ffs-cli -- mount /path/to/fs.img /mnt/ffs \
...
`RefreshLossModel` formally compares these policies via expected-loss calculations across workload profiles. Under heavy writes, the Hybrid policy achieves **83.3% reduction in p95 stale-window age** compared to age-only, because the block-count trigger caps staleness at ~500 writes regardless of how fast they arrive.
...
| Adaptive refresh hybrid trigger | Write-heavy group, age vs hybrid | **83.3% p95 stale-window reduction** vs age-only refresh |
...
├── Cargo.lock
├── README.md                   This file
├── CHANGELOG.md                Capability-area changelog (3,229 commits, 2026-02-09 → 2026-05-16)
...
# Mount with experimental read-write
...
|---|---|---|
| **Safe-Merge Conflict Arbitration** | Verified | 120-writer stress, SafeMerge 9.5× lower expected loss than Strict |
...
- **Testing.** ~7,419 `#[test]` / `proptest!` entries across 21 crates as of 2026-05-16 (7,380 `#[test]` + 39 `proptest!`; the count grows with every commit), 60 fuzz targets, 11 criterion benchmarks, 113 end-to-end gate scripts, metamorphic-relation proptests across the checksum/parser surface, and 167+ insta snapshots covering every emitted report shape.
...
|---|---|---|
| Core mutations (`create`, `mkdir`, `unlink`, `rmdir`, `rename`, `write`, `setattr`, `link`, `symlink`, xattrs) | Supported (experimental) | Deterministic success/error under `ffs-core` + FUSE tests, including explicit xattr mode semantics (`Create`/`Replace`) |
| `fallocate(mode=0, FALLOC_FL_KEEP_SIZE)` | Partially supported | Preallocation paths supported and validated |
| `fallocate(PUNCH_HOLE\|KEEP_SIZE)` | Supported (experimental) | Zero-fills the requested range while preserving file size and unaffected bytes |
| `fallocate(ZERO_RANGE [\|KEEP_SIZE])` | Supported (experimental) | Zero-fills; `KEEP_SIZE` preserves EOF; non-`KEEP_SIZE` can extend file size |
| `fallocate(COLLAPSE_RANGE)` | Supported (experimental) | Removes aligned range, shifts tail left, shrinks file size; preserves shifted prealloc extents as FIEMAP `UNWRITTEN` |
| `fallocate(INSERT_RANGE)` | Supported (experimental) | Inserts aligned hole, shifts tail right, grows file size; preserves shifted prealloc extents as FIEMAP `UNWRITTEN` |
