...
This workstream is the core fix for the headline problem, but only A0 was implemented. A1-A6 are designed and spec'd but remain in progress.

**A0 Safety Interlock (bd-xuo95.1).** *Implemented.* It immediately stopped the silent success. btrfs metadata-mutating FUSE ops now fail closed with `EROFS` unless the explicit `--btrfs-rw-ephemeral-ok` flag is passed. No code path may log `outcome="applied"` for a btrfs metadata change that is not on disk. E2E validation: `scripts/e2e/ffs_btrfs_rw_interlock_e2e.sh`.

**A1-A3 CoW Serialization Pipeline.** *Designed, not yet implemented.* The design spec in `docs/design-btrfs-metadata-writeback.md` covers:
...
**A4 Crash Consistency (bd-xuo95.5).** *Oracle infrastructure only.* The write-dependency DAG, WB-I1/WB-I2 invariant oracles, and DPOR enumeration are implemented. The crash matrix runs against in-memory simulation. When A1-A3 land, the matrix will validate real writeback.
...
**A5-A6 Remount/Differential Tests.** *Blocked on A1-A3.* Test definitions exist; they cannot pass until serialization is implemented.
...
The core btrfs durability implementation. The design spec exists (`docs/design-btrfs-metadata-writeback.md`) and invariant oracles exist, but the CoW node serializer, metadata block allocation, atomic root commit, and remount-persistence tests are not implemented. `FEATURE_PARITY.md` rows 88-90 track this as 🚧 "in progress." This is the main gap between "btrfs RW facade" and "btrfs RW durable."
