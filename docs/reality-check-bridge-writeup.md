...
**btrfs RW:** Not yet durable. The serializer + writeback-DAG + atomic-root-commit infrastructure ships (`DiskWritebackContext::serialize_node`, `WriteDependencyDag`, `WritebackExecutor`, `btrfs_full_transaction_commit`) and is unit-test green in isolation. Wiring it into the FUSE destroy hook so unmount persists the in-memory CoW tree was tried in this session and exposed a downstream gap: `DiskWritebackContext::block_to_bytenr` at `crates/ffs-btrfs/src/writeback.rs:533` uses the placeholder mapping `bytenr = block * nodesize` instead of allocating real btrfs *logical addresses* from the chunk tree. The first mounted-FUSE durability test (create fresh image → mount RW → write 5 files → unmount → remount) caught it empirically: the new superblock pointed `sb.root` at an offset not covered by any chunk, and the next mount failed with `invalid on-disk format: invalid field: logical_address (not covered by any chunk)`. That is strictly worse than the pre-fix silent-loss behavior — an unmountable image is harder to recover from than a fresh one — so the destroy-hook commit was reverted in the same session; btrfs writes are once again discarded with the mount, the image stays valid, and `FEATURE_PARITY.md` rows 88-90 are honestly 🚧 again. The follow-up work is filed as **bd-1ving**: thread `DiskWritebackContext` through `BtrfsAllocState::alloc_metadata_for_tree` so serialized nodes land at chunk-covered logical addresses.

**The honest summary:** btrfs RW remains a silent-data-loss surface, but is now also empirically *known* to be one — the mounted-FUSE durability harness exists and runs, and the next step (`bd-1ving`) has a specific failing test to target. The `--btrfs-rw-ephemeral-ok` flag keeps its tree-log fast-fsync codepath; nothing in the writeback layer pretends to be durable today.
