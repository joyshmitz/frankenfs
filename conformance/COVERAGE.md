...
|-----|-----------------|--------------|--------------|------------------|---------------------|--------------------|-------------------------|---------------|--------------------|----------|----------------------------------------|
| OEI-001 | conformance/COVERAGE.md#next-actions | Consider additional edge cases | Parser and mutation boundary cases can be lost behind a green aggregate coverage score | Coverage matrix lists covered structures and adversarial parser rows, and the fixed seed manifest now pins representative parser outcomes | fuzz_smoke | `fuzz_smoke::tests::default_manifest_validates_and_runs` plus manifest parser, expected-class, panic, timeout, and artifact-contract unit tests | `ffs-harness validate-fuzz-smoke --out artifacts/fuzz-smoke/fuzz_smoke_report.json` and `scripts/e2e/ffs_fuzz_smoke_e2e.sh` emit fixed seed IDs, corpus checksum, duration, stdout/stderr paths, cleanup status, and target summary | source_path; row_id; decision; artifact_paths; reproduction_command | report_json; run_log; source_path; row_id | completed_artifact | bd-rchk7.4 / `tests/fuzz-smoke/fuzz_smoke_manifest.json`, `crates/ffs-harness/src/fuzz_smoke.rs`, `scripts/e2e/ffs_fuzz_smoke_e2e.sh` |
| OEI-002 | conformance/COVERAGE.md#next-actions | Continue targeted adversarial corpus expansion for remaining mutation surfaces | Remaining mutation surfaces need explicit corpus or fixture ownership instead of open prose | Btrfs delayed refs, transactions, allocator, and COW mutation rows are covered; repair corpus metadata is decoded by the fuzz-smoke manifest, while deeper repair corpus follow-up remains owned by `bd-rchk7.2` | repair_corpus | Inventory row validation here plus repair corpus fixture-manifest tests in `bd-rchk7.2` | Repair/ledger corpus metadata is included as fixed seed `fs_repair_corpus_manifest` in `tests/fuzz-smoke/fuzz_smoke_manifest.json` | source_path; row_id; decision; artifact_paths; reproduction_command | report_json; run_log; source_path; row_id | linked_bead | bd-rchk7.2 open; bd-rchk7.4 fixed-seed smoke current |
| OEI-003 | tests/fuzz_corpus/README.md#coverage-intent | Every sample is processed by ext4 and btrfs parser entry points without panics | Existing deterministic corpus intent must stay tied to executable parser checks | `crates/ffs-ondisk/tests/adversarial_corpus.rs` owns the full corpus count and parser no-panic oracle; `bd-rchk7.4` adds a smaller artifact-producing smoke subset | parser_unit | `open_ended_inventory::tests::parses_inventory_rows` validates completed-artifact rows and field vocabulary | Existing adversarial corpus test plus `ffs-harness validate-fuzz-smoke` fixed-seed report | source_path; row_id; decision; artifact_paths; reproduction_command | report_json; run_log; source_path; row_id | completed_artifact | tests/fuzz_corpus/README.md, crates/ffs-ondisk/tests/adversarial_corpus.rs, and tests/fuzz-smoke/fuzz_smoke_manifest.json / current |
...
- [x] OEI-001 / `bd-rchk7.4` tracks additional parser edge cases as fixed-seed fuzz-smoke work with log and artifact requirements - DONE
...
- [ ] OEI-002 / `bd-rchk7.2` tracks remaining deep repair corpus expansion; `bd-rchk7.4` now covers deterministic fuzz-smoke expansion with fixed seeds and artifact requirements.
