...
python3 - "$MANIFEST_JSON" "$BAD_CAP_JSON" "$BAD_ENV_JSON" "$BAD_ARTIFACT_JSON" "$BAD_UNIT_JSON" <<'PY'
from __future__ import annotations
...
source, bad_cap, bad_env, bad_artifact, bad_unit = map(pathlib.Path, sys.argv[1:])
...
invalid_failures=0
for bad in "$BAD_CAP_JSON" "$BAD_ENV_JSON" "$BAD_ARTIFACT_JSON" "$BAD_UNIT_JSON"; do
    if cargo run --quiet -p ffs-harness -- validate-performance-baseline-manifest \
        --manifest "$bad" \
