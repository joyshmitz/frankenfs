...
SELF_CHECK="${FFS_CASEFOLD_CORPUS_SELF_CHECK:-0}"
SKIP_SELF_CHECK="${FFS_CASEFOLD_CORPUS_SKIP_SELF_CHECK:-0}"
...
fixture_case="${FFS_CASEFOLD_CORPUS_FIXTURE_CASE:-complete}"
...
{
  "schema_version": 1,
  "corpus_id": "frankenfs_ext4_casefold_corpus_v1",
  "valid": true,
  "case_count": 12,
  "kernel_compared_count": 2,
  "operations_seen": [
    "lookup",
    "create",
    "rename",
...
case "$fixture_case" in
    ...
    complete)
        ;;
    *)
        echo "unknown casefold corpus fixture case: $fixture_case" >&2
        exit 64
        ;;
esac
...
case "$command_text" in
    *"cargo test -p ffs-harness --lib casefold_corpus"*)
        printf '%s\n' \
            "test default_corpus_validates_required_coverage ... ok" \
            "test render_markdown_summarizes_default_corpus ... ok" \
            "test malformed_source_hex_is_rejected ... ok"
        exit 0
        ;;
    *"casefold_corpus_invalid.json"*)
        echo "failed to parse casefold corpus JSON: missing field source_name_bytes_hex" >&2
        exit 1
        ;;
    *"--format markdown"*)
        printf '%s\n' \
            "# Ext4 Casefold Corpus" \
            "" \
            "- corpus: \`frankenfs_ext4_casefold_corpus_v1\`" \
            "- outcome: \`invalid_encoding_refused\`" \
...
run_fixture_child() {
    ...
    local child_log="$E2E_LOG_DIR/casefold_corpus_fixture_${fixture_case}.log"

    set +e
    FFS_E2E_DISABLE_TEMP_CLEANUP=1 \
        FFS_CASEFOLD_CORPUS_SELF_CHECK=0 \
        FFS_CASEFOLD_CORPUS_SKIP_SELF_CHECK=1 \
        FFS_CASEFOLD_CORPUS_FIXTURE_CASE="$fixture_case" \
        RCH_BIN="$stub_path" \
        "$REPO_ROOT/scripts/e2e/ffs_casefold_corpus_e2e.sh" >"$child_log" 2>&1
    local child_status=$?
    set -e
...
run_self_check() {
    ...
    e2e_step "Deterministic ext4 casefold corpus wrapper self-check"
    local stub_path child_info child_status child_log result_path report_path markdown_path
    stub_path="$E2E_LOG_DIR/rch-casefold-corpus-fixture"
    ...
    report_path="$(dirname "$result_path")/casefold_corpus_report.json"
    markdown_path="$(dirname "$result_path")/casefold_corpus_markdown.raw"
    if [[ "$child_status" == "0" ]] \
        && [[ -n "$result_path" ]] \
        && [[ -f "$report_path" ]] \
        && [[ -f "$markdown_path" ]] \
        && jq -e '
            .verdict == "PASS"
            and ([.scenarios[] | select(.scenario_id == "casefold_corpus_validates" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "casefold_corpus_coverage" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "casefold_corpus_invalid_rejected" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "casefold_corpus_markdown_docs" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "casefold_corpus_unit_tests" and .outcome == "PASS")] | length == 1)
        ' "$result_path" >/dev/null \
        && jq -e '
            .valid == true
            and .case_count >= 10
            and .kernel_compared_count >= 1
            and ((.operations_seen | index("lookup")) != null)
            and ((.operations_seen | index("create")) != null)
            and ((.operations_seen | index("rename")) != null)
            and ((.operations_seen | index("cross_directory_rename")) != null)
            and ((.operations_seen | index("mount_feature_check")) != null)
            and ((.outcomes_seen | index("create_collision_refused")) != null)
            and ((.outcomes_seen | index("rename_collision_refused")) != null)
            and ((.outcomes_seen | index("invalid_encoding_refused")) != null)
            and ((.outcomes_seen | index("mount_feature_accepted")) != null)
        ' "$report_path" >/dev/null \
        && grep -q "# Ext4 Casefold Corpus" "$markdown_path" \
        && grep -q "invalid_encoding_refused" "$markdown_path"; then
        scenario_result "casefold_corpus_fixture_complete_self_check" "PASS" "result=${result_path} report=${report_path} markdown=${markdown_path}"
    else
        scenario_result "casefold_corpus_fixture_complete_self_check" "FAIL" "log=${child_log}"
        e2e_fail "casefold corpus complete fixture self-check failed"
    fi
    ...
    if [[ "$child_status" != "0" ]] \
        ...
        scenario_result "casefold_corpus_fixture_local_fallback_self_check" "PASS" "result=${result_path}"
    else
        scenario_result "casefold_corpus_fixture_local_fallback_self_check" "FAIL" "log=${child_log}"
        e2e_fail "casefold corpus local fallback fixture self-check failed"
    fi
}

if [[ "$SELF_CHECK" == "1" ]]; then
    run_self_check
    e2e_pass "casefold corpus wrapper self-check"
    exit 0
fi
