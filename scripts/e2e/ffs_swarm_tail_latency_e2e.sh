...
RCH_CAPTURE_VISIBILITY="${FFS_SWARM_TAIL_RCH_VISIBILITY:-${RCH_VISIBILITY:-summary}}"
...
SELF_CHECK="${FFS_SWARM_TAIL_LATENCY_SELF_CHECK:-0}"
SKIP_SELF_CHECK="${FFS_SWARM_TAIL_LATENCY_SKIP_SELF_CHECK:-0}"
...
fixture_case="${FFS_SWARM_TAIL_LATENCY_FIXTURE_CASE:-complete}"
    ...
    cat <<'JSON'
{
  ...
  "component_dominance_alert_count": 3,
...
emit_markdown_report() {
    cat <<'MD'
# Swarm Tail-Latency Ledger

## Tail Attribution

- `wal_fsync`: dominant component
- `fuse_wrapper`: watched component
- `missing_reference`: experimental downgrade preserved
MD
}
...
case "$fixture_case" in
    ...
    complete)
        ;;
    *)
        echo "unknown swarm tail-latency fixture case: $fixture_case" >&2
        exit 64
        ;;
esac
...
case "$command_text" in
    *"cargo test -p ffs-harness swarm_tail_latency"*)
        printf '%s\n' \
            "test checked_in_swarm_tail_latency_ledger_validates ... ok" \
            "test render_swarm_tail_latency_markdown_checked_in_ledger_snapshot ... ok" \
            "test swarm_tail_latency_report_json_shape ... ok"
        exit 0
        ;;
    *"swarm_tail_bad_component.json"*)
        echo "error: missing required component wal_fsync" >&2
        exit 1
        ;;
    *"swarm_tail_bad_reference.json"*)
        echo "error: public performance wording must remain experimental" >&2
        exit 1
        ;;
    *"swarm_tail_bad_host.json"*)
        echo "error: host.host_fingerprint must not be empty" >&2
        exit 1
        ;;
    *"swarm_tail_bad_bucket.json"*)
        echo "error: p50 <= p95 <= p99 invariant violated" >&2
        exit 1
        ;;
    *"--format markdown"*)
        emit_markdown_report
...
run_fixture_child() {
    ...
    local child_log="$E2E_LOG_DIR/swarm_tail_fixture_${fixture_case}.log"

    set +e
    FFS_E2E_DISABLE_TEMP_CLEANUP=1 \
        FFS_SWARM_TAIL_LATENCY_SELF_CHECK=0 \
        FFS_SWARM_TAIL_LATENCY_SKIP_SELF_CHECK=1 \
        FFS_SWARM_TAIL_LATENCY_FIXTURE_CASE="$fixture_case" \
        RCH_BIN="$stub_path" \
        RCH_COMMAND_TIMEOUT_SECS=8 \
        RCH_ARTIFACT_RETRIEVAL_GRACE_SECS=1 \
        "$REPO_ROOT/scripts/e2e/ffs_swarm_tail_latency_e2e.sh" >"$child_log" 2>&1
    local child_status=$?
    set -e
...
run_self_check() {
    ...
    e2e_step "Deterministic swarm tail-latency wrapper self-check"
    local stub_path child_info child_status child_log result_path report_path markdown_path unit_log
    stub_path="$E2E_LOG_DIR/rch-swarm-tail-fixture"
    ...
    report_path="$(dirname "$result_path")/swarm_tail_latency_report.json"
    markdown_path="$(dirname "$result_path")/swarm_tail_latency_report.md"
    ...
    if [[ "$child_status" == "0" ]] \
        ...
        && jq -e '
            .verdict == "PASS"
            and ([.scenarios[] | select(.scenario_id == "swarm_tail_unit_tests" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "swarm_tail_default_ledger" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "swarm_tail_markdown" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "swarm_tail_missing_component_fail_closed" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "swarm_tail_missing_reference_fail_closed" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "swarm_tail_missing_host_fail_closed" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "swarm_tail_nonmonotonic_bucket_fail_closed" and .outcome == "PASS")] | length == 1)
        ' "$result_path" >/dev/null \
        && jq -e '
            .valid == true
            and .row_count == 5
            and .missing_reference_count == 1
            and .component_dominance_alert_count >= 3
            and .classification_counts.pass == 1
            and .classification_counts.warn == 1
            and .classification_counts.fail == 1
            and .classification_counts.noisy == 1
            and .classification_counts.missing_reference == 1
        ' "$report_path" >/dev/null \
        && grep -q "# Swarm Tail-Latency Ledger" "$markdown_path" \
        && grep -q "Tail Attribution" "$markdown_path" \
        && grep -q "missing_reference" "$markdown_path" \
        && grep -q "checked_in_swarm_tail_latency_ledger_validates" "$unit_log"; then
        scenario_result "swarm_tail_fixture_complete_self_check" "PASS" "result=${result_path} report=${report_path} markdown=${markdown_path}"
    else
        scenario_result "swarm_tail_fixture_complete_self_check" "FAIL" "log=${child_log}"
        e2e_finish "FAIL" "swarm tail-latency complete fixture self-check failed"
        exit 1
    fi
    ...
    if [[ "$child_status" != "0" ]] \
        ...
        scenario_result "swarm_tail_fixture_local_fallback_self_check" "PASS" "result=${result_path}"
    else
        scenario_result "swarm_tail_fixture_local_fallback_self_check" "FAIL" "log=${child_log}"
        e2e_finish "FAIL" "swarm tail-latency local fallback fixture self-check failed"
        exit 1
    fi
}
...
if [[ "$SELF_CHECK" == "1" ]]; then
    run_self_check
    e2e_finish "PASS" "swarm tail-latency wrapper self-check"
    exit 0
fi
