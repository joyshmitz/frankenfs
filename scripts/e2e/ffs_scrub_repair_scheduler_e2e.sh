...
RCH_CAPTURE_VISIBILITY="${FFS_SCRUB_REPAIR_SCHEDULER_RCH_VISIBILITY:-${RCH_VISIBILITY:-summary}}"
...
if [[ "${FFS_SCRUB_REPAIR_SCHEDULER_RESULT_SELF_CHECK_ONLY:-0}" == "1" ]]; then
    e2e_step "Result summary merge self-check"
    scenario_result "scrub_repair_scheduler_summary_merge" "PASS" "custom and shared result fields merge"
    e2e_log "SCENARIO_RESULT|scenario_id=too_short|outcome=PASS"
    e2e_log "SCENARIO_RESULT|scenario_id=scrub_repair_scheduler_summary_merge|outcome=PASS|bad_field"
    e2e_log "SCENARIO_RESULT|scenario_id=scrub_repair_scheduler_summary_merge|outcome=PASS|"
    ...
    ' "$E2E_LOG_DIR/result.json" >/dev/null; then
        e2e_log "Scrub/repair scheduler result summary merge self-check passed"
        exit 0
    fi
    jq . "$E2E_LOG_DIR/result.json" || true
    e2e_log "Scrub/repair scheduler result summary merge self-check failed"
    exit 1
fi
