...
SELF_CHECK="${FFS_BTRFS_MULTIDEVICE_CORPUS_SELF_CHECK:-0}"
SKIP_SELF_CHECK="${FFS_BTRFS_MULTIDEVICE_CORPUS_SKIP_SELF_CHECK:-0}"
...
fixture_case="${FFS_BTRFS_MULTIDEVICE_CORPUS_FIXTURE_CASE:-complete}"
...
{
  "schema_version": 1,
  "corpus_id": "frankenfs_btrfs_multidevice_corpus_v1",
  "valid": true,
  "scenario_count": 6,
  "scenario_kinds": [
    "healthy_assembly",
...
case "$fixture_case" in
    ...
    complete)
        ;;
    *)
        echo "unknown btrfs multidevice corpus fixture case: $fixture_case" >&2
        exit 64
        ;;
esac
...
case "$command_text" in
    *"cargo test -p ffs-harness --lib btrfs_multidevice_corpus"*)
        printf '%s\n' \
            "test default_corpus_validates_required_kinds ... ok" \
            "test render_markdown_summarizes_default_corpus ... ok" \
            "test malformed_device_image_hash_is_rejected ... ok"
        exit 0
        ;;
    *"btrfs_multidevice_corpus_invalid.json"*)
        echo "failed to parse btrfs multi-device corpus JSON: missing field image_hash" >&2
        exit 1
        ;;
    *"--format markdown"*)
        printf '%s\n' \
            "# Btrfs Multi-Device Corpus" \
            "" \
            "- corpus: \`frankenfs_btrfs_multidevice_corpus_v1\`" \
...
run_fixture_child() {
    ...
    local child_log="$E2E_LOG_DIR/btrfs_multidevice_corpus_fixture_${fixture_case}.log"

    set +e
    FFS_E2E_DISABLE_TEMP_CLEANUP=1 \
        FFS_BTRFS_MULTIDEVICE_CORPUS_SELF_CHECK=0 \
        FFS_BTRFS_MULTIDEVICE_CORPUS_SKIP_SELF_CHECK=1 \
        FFS_BTRFS_MULTIDEVICE_CORPUS_FIXTURE_CASE="$fixture_case" \
        RCH_BIN="$stub_path" \
        "$REPO_ROOT/scripts/e2e/ffs_btrfs_multidevice_corpus_e2e.sh" >"$child_log" 2>&1
    local child_status=$?
    set -e
...
run_self_check() {
    ...
    e2e_step "Deterministic btrfs multidevice corpus wrapper self-check"
    local stub_path child_info child_status child_log result_path report_path markdown_path
    stub_path="$E2E_LOG_DIR/rch-btrfs-multidevice-corpus-fixture"
    ...
    report_path="$(dirname "$result_path")/btrfs_multidevice_corpus_report.json"
    markdown_path="$(dirname "$result_path")/btrfs_multidevice_corpus_markdown.raw"
    if [[ "$child_status" == "0" ]] \
        && [[ -n "$result_path" ]] \
        && [[ -f "$report_path" ]] \
        && [[ -f "$markdown_path" ]] \
        && jq -e '
            .verdict == "PASS"
            and ([.scenarios[] | select(.scenario_id == "btrfs_multidevice_corpus_validates" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "btrfs_multidevice_corpus_coverage" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "btrfs_multidevice_corpus_invalid_rejected" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "btrfs_multidevice_corpus_markdown_docs" and .outcome == "PASS")] | length == 1)
            and ([.scenarios[] | select(.scenario_id == "btrfs_multidevice_corpus_unit_tests" and .outcome == "PASS")] | length == 1)
        ' "$result_path" >/dev/null \
        && jq -e '
            .valid == true
            and .scenario_count >= 6
            and ((.scenario_kinds | index("healthy_assembly")) != null)
            and ((.scenario_kinds | index("device_order_permutation")) != null)
            and ((.scenario_kinds | index("missing_device")) != null)
            and ((.scenario_kinds | index("duplicate_device_id")) != null)
            and ((.scenario_kinds | index("stale_superblock")) != null)
            and ((.scenario_kinds | index("unsupported_profile")) != null)
            and ((.profiles | index("raid1")) != null)
            and ((.profiles | index("raid5")) != null)
        ' "$report_path" >/dev/null \
        && grep -q "# Btrfs Multi-Device Corpus" "$markdown_path" \
        && grep -q "unsupported_profile" "$markdown_path"; then
        scenario_result "btrfs_multidevice_corpus_fixture_complete_self_check" "PASS" "result=${result_path} report=${report_path} markdown=${markdown_path}"
    else
        scenario_result "btrfs_multidevice_corpus_fixture_complete_self_check" "FAIL" "log=${child_log}"
        e2e_fail "btrfs multidevice corpus complete fixture self-check failed"
    fi
    ...
    if [[ "$child_status" != "0" ]] \
        ...
        scenario_result "btrfs_multidevice_corpus_fixture_local_fallback_self_check" "PASS" "result=${result_path}"
    else
        scenario_result "btrfs_multidevice_corpus_fixture_local_fallback_self_check" "FAIL" "log=${child_log}"
        e2e_fail "btrfs multidevice corpus local fallback fixture self-check failed"
    fi
}

if [[ "$SELF_CHECK" == "1" ]]; then
    run_self_check
    e2e_pass "btrfs multidevice corpus wrapper self-check"
    exit 0
fi
